...
const FeaturedImageUpload = ({ onChange, value }: TFeatureImageProps): ReactElement => {
    ...
    const handleOnSelectImage = ({ id, url }: { id: number; url: string }) => {
        if (selectedImage.some(image => image.id === id)) {
            setSelectedImage(selectedImage.filter(image => image.id !== id));
        } else {
            setSelectedImage([...selectedImage, { id, url }]);
        }
    };
                    ...
                    {selectedImage.map(image => (
                        <li key={image.id}>
                            <ImageThumbnailWithAction
                                url={image.url}
                                actions={
                                    <button type="button" onClick={() => handleRemoveImage(image.id)} className="btn btn-sm btn-primary">
                    ...
                    <ul className="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-3 sm:gap-x-4">
                        {IMAGES.map(({ id, url }) => (
                            <ImageCard
                                ...
                                id={id}
                                url={url}
                                isChecked={selectedImage.some(image => image.id === id)}
